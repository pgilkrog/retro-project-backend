...
import { InstalledProgram } from '../models/InstalledProgram'
...
router.get('/installedProgram/:id', auth, async (req: Request, res: Response) => {
  try {
    const fetchedItems = await InstalledProgram.find({ userId: req.params.id })
    res.json(fetchedItems)
  } catch (error: any) {
...
router.post('/installedProgram', auth, jsonParser, async (req: Request, res: Response) => {
...
// @route       DELETE api/program
// @desc        Delete program by id
router.delete('/installedProgram/:id', auth, async (req: Request, res: Response) => {
  try {
    console.log('DELETE INSTALLED PROGRAM', req.params.id)
    const deletedItem = await InstalledProgram.findByIdAndDelete({ _id: req.params.id })
    res.send({ item: deletedItem })
  } catch (error: any) {
    console.log(error.message)
    res.status(500).send('server error')
...
router.put('/installedProgram/:id', auth, jsonParser, async (req: Request, res: Response) => {
  ...
  try {
    const updateProgram = await InstalledProgram.findByIdAndUpdate(id, programToUpdate, { new: true})
